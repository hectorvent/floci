...
class CloudWatchLogsMetricFilterPublishingTest {
    ...
    private List<MetricDatum> published(String namespace) {
        ...
        verify(metrics).putMetricData(eq(namespace), datums.capture(), eq(REGION));
    ...
    void aMatchingEventPublishesTheLiteralValueAtTheEventTime() {
        ...
        service.onLogEventsIngested(new LogEventsIngested(REGION, GROUP, "s1", List.of(
    ...
    void nothingIsPublishedWithoutADefaultWhenNothingMatchedOrForOtherGroups() {
        ...
        service.onLogEventsIngested(new LogEventsIngested(REGION, GROUP, "s1", List.of(event(1L, "[INFO] fine"))));
        service.onLogEventsIngested(new LogEventsIngested(REGION, OTHER_GROUP, "s1", List.of(event(1L, "[ERROR] boom"))));
        service.onLogEventsIngested(new LogEventsIngested("eu-west-1", GROUP, "s1", List.of(event(1L, "[ERROR] boom"))));

        verify(metrics, never()).putMetricData(anyString(), anyList(), anyString());
    }
    ...
    void everyFilterOfTheGroupPublishesAndAFailingOneDoesNotStopTheOthers() {
        ...
        doThrow(new IllegalStateException("store closed")).when(metrics).putMetricData(eq("Broken"), any(), eq(REGION));

        service.onLogEventsIngested(new LogEventsIngested(REGION, GROUP, "s1", List.of(event(1L, "[ERROR] boom"))));
